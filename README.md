...
> **Status / 상태** — **v0.6.0** (2026-08-20). A from-scratch MIT ORB interoperates with omniORB 4.3.4 and JacORB 3.9 **in both directions** at GIOP 1.0/1.1/1.2. v0.5.0 was the release a peer's bytes produced; **this one is the release a plan reviewed against the code produced** — three reviewers read every remaining-work, status and risk row against the tree and found progress wrong in both directions (nineteen rows understated, six overstated), and the batches sent after what they found closed five defects nobody had asked about: the §5.3 comparison had **never run** over a guarded multi-file contract; a `valuetype` and an abstract interface went on the wire as **object references**; `--repair-prompt` gave a model the wrong file's line; a permanent forward moved one handle and not its clones; and the front end diverged from the oracle in **both** directions on constants. Twice, a batch sent to fix a keyword found a production. Six CORBA services on our own POA with every declared operation measured over the wire and the coverage document **generated from the sweep**; what remains, and which of it cannot be measured here, is one document (D010) whose class-B rows are counted `SKIPPED` groups naming their fixture. Structure: [`ARCHITECTURE.md`](docs/ARCHITECTURE.md) · Measurements: [`COMPONENTS.md`](docs/COMPONENTS.md) · Changes: [`CHANGELOG.md`](CHANGELOG.md).
> **v0.6.0** (2026-08-20). 이번 릴리즈는 **계획서를 코드에 대조한 검토**가 만들었습니다 — 진행률이 양방향으로 틀려 있었고(19행 과소, 6행 과대), 그 검토가 드러낸 것을 좇아 보낸 배치들이 아무도 요청하지 않은 결함 다섯을 닫았습니다: 가드가 있는 다중 파일 계약에서 §5.3 비교가 **한 번도 실행되지 않았고**, valuetype과 추상 인터페이스가 **객체 참조로** 와이어에 나갔으며, `--repair-prompt`가 모델에게 엉뚱한 파일의 줄을 주었고, 영속 포워드가 한 핸들만 옮겼으며, 상수에서 프런트엔드가 오라클과 **양방향으로** 어긋나 있었습니다. 키워드를 고치라고 보낸 배치가 두 번 모두 프로덕션을 찾았습니다.
