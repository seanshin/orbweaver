...
> **Status / 상태** — **v0.3.0** (2026-08-14). Phases 0–3.5 complete, Phase 4 substantially landed, Phase 5 half landed, Phase 6 opened (IOR rewriting for NAT/containers). A from-scratch MIT ORB interoperates with omniORB 4.3.4 and JacORB 3.9 **in both directions** at GIOP 1.0/1.1/1.2 — and so does generated code: omniORB's python client drives a **generated server skeleton**, JacORB's Interface Repository serves our **ingestion** client, and omniORB resolved a `corbaname:` URL our own `to_url` built. Six CORBA services on our own POA (Naming, Event, Trading, a read-only Interface Repository, LifeCycle/tenancy, and the MoE control plane), with every declared operation of each **measured over the wire** ([`SERVICES-COVERAGE.md`](docs/SERVICES-COVERAGE.md)). Structure: [`ARCHITECTURE.md`](docs/ARCHITECTURE.md) · Measurements: [`COMPONENTS.md`](docs/COMPONENTS.md) · Changes: [`CHANGELOG.md`](CHANGELOG.md).
> **v0.3.0** (2026-08-14). Phase 0–3.5 완료, Phase 4 대부분, Phase 5 절반, Phase 6 착수. 생성된 코드도 양방향으로 측정됩니다 — omniORB의 python 클라이언트가 **생성된 서버 스켈레톤**을 구동하고, JacORB의 인터페이스 리포지토리가 우리 **수집** 클라이언트에 서빙하며, omniORB가 우리 `to_url`이 만든 `corbaname:` URL을 resolve합니다. 우리 POA 위의 CORBA 서비스 여섯 개는 **선언된 모든 오퍼레이션이 와이어에서 측정**되어 있습니다.
