...
> **Status / 상태** — **v0.5.0** (2026-08-19). Phases 0–3.5 complete, Phase 4 landed, Phase 5 half landed, Phase 6 opened. A from-scratch MIT ORB interoperates with omniORB 4.3.4 and JacORB 3.9 **in both directions** at GIOP 1.0/1.1/1.2 — and this release is the one **their bytes** produced: twelve wire-behaviour changes, every one invisible to our own round trip, each found by recording a peer's octets with provenance and re-taking the capture on every harness run (union labels, `long double`, UTF-16 order, an encapsulation's origin, a 1.1 `wstring`'s mark, a union `default:` label, the member list, a 1.2 `wchar` that is itself a mark). Both forward statuses are distinguished by behaviour, ours and omniORB's alike; a servant can say *permanent*. AnyJSON v1.1 describes a type structurally on both targets, and the negotiated codeset reaches the marshaller (D008, D009). Six CORBA services on our own POA, every declared operation measured over the wire and the coverage document **generated from the sweep** ([`SERVICES-COVERAGE.md`](docs/SERVICES-COVERAGE.md)). What remains, and which of it cannot be measured here, is one document (D010) whose class-B rows are counted `SKIPPED` groups naming their fixture. Structure: [`ARCHITECTURE.md`](docs/ARCHITECTURE.md) · Measurements: [`COMPONENTS.md`](docs/COMPONENTS.md) · Changes: [`CHANGELOG.md`](CHANGELOG.md).
> **v0.5.0** (2026-08-19). Phase 0–3.5 완료, Phase 4 착지, Phase 5 절반, Phase 6 착수. 이번 릴리즈는 **피어의 바이트**가 만들었습니다 — 와이어 동작 변경 열둘, 전부 우리 왕복으로는 보이지 않던 것으로, 피어의 옥텟을 출처와 함께 기록하고 하네스가 돌 때마다 재채취해서 찾았습니다(union 레이블, `long double`, UTF-16 순서, 인캡슐레이션 원점, 1.1 `wstring`의 표식, union `default:` 레이블, 멤버 목록, 표식 그 자체인 1.2 `wchar`). 두 포워드 상태를 우리 클라이언트와 omniORB가 같은 동작으로 구별하고, 서번트가 *permanent*를 말할 수 있습니다. AnyJSON v1.1은 두 타깃 모두에서 타입을 구조로 서술하고, 협상된 코드셋이 마샬러에 닿습니다(D008, D009). 우리 POA 위의 CORBA 서비스 여섯 개는 모든 오퍼레이션이 와이어에서 측정되며 커버리지 문서는 **스윕이 생성**합니다. 남은 것과 여기서 잴 수 없는 것은 문서 하나(D010)이고, 그 B류 행은 빠진 픽스처를 이름 붙인 `SKIPPED` 그룹입니다.
