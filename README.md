...
> **Status / 상태** — **v0.4.0** (2026-08-14). Phases 0–3.5 complete, Phase 4 substantially landed, Phase 5 half landed, Phase 6 opened (IOR rewriting for NAT/containers). A from-scratch MIT ORB interoperates with omniORB 4.3.4 and JacORB 3.9 **in both directions** at GIOP 1.0/1.1/1.2 — and so does generated code: omniORB's python client drives a **generated server skeleton**, JacORB's Interface Repository serves our **ingestion** client, and omniORB resolved a `corbaname:` URL our own `to_url` built. Six CORBA services on our own POA (Naming, Event, Trading, a read-only Interface Repository, LifeCycle/tenancy, and the MoE control plane), with every declared operation of each **measured over the wire** ([`SERVICES-COVERAGE.md`](docs/SERVICES-COVERAGE.md)). Contracts arrive the way they actually arrive: `#include` resolves, and a **thirteen-file legacy estate** — four prefix styles, nothing annotated — runs the whole path as a gate. Two targets now, Rust and Python, because one emitter is one opinion. Structure: [`ARCHITECTURE.md`](docs/ARCHITECTURE.md) · Measurements: [`COMPONENTS.md`](docs/COMPONENTS.md) · Changes: [`CHANGELOG.md`](CHANGELOG.md).
> **v0.4.0** (2026-08-14). Phase 0–3.5 완료, Phase 4 대부분, Phase 5 절반, Phase 6 착수. 생성된 코드도 양방향으로 측정됩니다 — omniORB의 python 클라이언트가 **생성된 서버 스켈레톤**을 구동하고, JacORB의 인터페이스 리포지토리가 우리 **수집** 클라이언트에 서빙하며, omniORB가 우리 `to_url`이 만든 `corbaname:` URL을 resolve합니다. 우리 POA 위의 CORBA 서비스 여섯 개는 **선언된 모든 오퍼레이션이 와이어에서 측정**되어 있습니다. 계약은 실제로 도착하는 모양대로 받습니다 — `#include`가 해석되고, **13개 파일짜리 레거시 estate**가 전 경로를 게이트로 돌립니다. 이미터 하나는 의견 하나이므로 타깃은 이제 Rust와 Python 둘입니다.
