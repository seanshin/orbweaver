...
> **Status / 상태** — **v0.7.0** (2026-08-26). A from-scratch MIT ORB interoperates with omniORB 4.3.4 and JacORB 3.9 **in both directions** at GIOP 1.0/1.1/1.2. v0.6.0 was the release a plan reviewed against the code produced; **this one is the release in which "finished" stopped being a feeling** — the owner replaced a list of capabilities with a property, and its home is [`D029`](docs/decisions/D029-what-a-complete-orb-would-mean.md) §6: *a caller can invoke any target holding only a reference, knowing nothing of its location, backend, language or load state, and this survives targets being added, removed, moved, loaded or evicted at runtime.* The harness now declares, per group, which transparency it bears on and prints what it could **not** measure in the group's own words; no score is derived from that ledger and none should be. A foreign ORB now forwards our client to a second process at a different address — every `LOCATION_FORWARD` this ORB had followed, it had written itself — the servant seam stopped being one language's and an object reference crosses it, and `Dispatch::knows`'s accept-every-key default was decided wrong because it enacts §15.3.8.6's `USE_DEFAULT_SERVANT` **by omission**. Two batches deliberately did **not** move their row and said why, which is worth more here than moving it. What is still open is a row in D029 §6.1 or a counted `SKIPPED` naming its fixture, never an omission. Structure: [`ARCHITECTURE.md`](docs/ARCHITECTURE.md) · Measurements: [`COMPONENTS.md`](docs/COMPONENTS.md) · Changes: [`CHANGELOG.md`](CHANGELOG.md).
> **v0.7.0** (2026-08-26). 이번 릴리즈에서 **"완성"이 느낌이기를 그만두었습니다.** 능력의 목록은 늘어나기만 하지만 성질은 구멍을 뚫어 볼 수 있습니다 — 소유자가 정한 0순위 기준의 집은 D029 §6이며 여기서 다시 적지 않습니다. 하네스는 이제 그룹마다 자기가 어느 투명성에 닿는지 선언하고, **재지 못한 것을 그룹 자신의 말로** 출력합니다. 점수는 내지 않으며 내서도 안 됩니다. 외부 ORB가 우리 클라이언트를 다른 주소의 두 번째 프로세스로 넘기고(이 ORB가 따라간 모든 포워드는 이 ORB가 쓴 것이었습니다), 서번트 심이 한 언어의 것이기를 그만두었으며 객체 참조가 심을 건넙니다. `knows`의 기본값은 명세가 누락으로는 도달할 수 없게 만들어 둔 정책을 **누락으로** 시행하고 있었습니다. 두 배치는 자기 행을 일부러 움직이지 않고 그 이유를 적었는데, 여기서는 그것이 움직이는 것보다 값이 나갑니다.
