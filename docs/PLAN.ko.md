...
- **무엇:** ~~`orbweaver-forge`의 S1 수집, S2 합성, S3 주석~~(착지 — 각각 생산자와
  ...
  인프로세스 라운드, 원인별 프롬프트); SIDL 어휘 v1은 두 크레이트에 미러된 상수 하나로
  확정되고 고정 테스트가 있다 — **버전 표식이 열린 항목**(`SIDL_VERSION`, v2 키를
  소비자가 구별할 수 있도록); 그리고 릴리스마다의 라이브 실행(§8 *AI 품질*)은 실행
...
- **무엇:** ~~SSLIOP/TLS 전송~~(기본 꺼짐 기능 뒤로 착지, 피어 증명은 BLOCKED
  ...
  미구축, 발급자가 필요, D010 B2); 강제 불가 대상의 카탈로그 표기(결정은
  `Assertion::RecordedOnly`로 존재; 카탈로그의 피어별 기록이 열린 항목, 두 피어로
  여기서 측정 가능).
...
- **무엇:** ~~`search_interfaces` 뒤의 임베딩 인덱스와 시맨틱 검색~~(착지: 인덱스,
  ...
  중심의 거버넌스 워크플로 — 오늘은 플래그와 출력 한 줄뿐, 저장소도 승인자도 없다;
  베낄 모양은 영속하는 `forge-pipeline --supersede`.
...
| **R18** | **피어의 결함이 우리의 규격이 된다** — 표준이 아니라 두 피어가 받아 주는 것에서 정한 와이어 선택(2026-08-19: union `default:` 레이블은 판별자 폭의 0인데, JacORB가 0이어야 하는 옥텟 하나로 읽고 omniORB는 값을 무시하기 때문; §9.3.5.1.4는 그 값에 의미가 없다고 한다) | 높음 — 두 결함 모두 살아 있다 | 중 — `any` 안의 모든 default 있는 union에 보임 | 기록 하네스는 그 두 피어에만 재확인하므로 JacORB가 리더를 고쳐도 초록이고 셋째 ORB에 대해 아무 말도 못 한다. 완화: 피어가 시험할 수 없는 *규격 적합* 절반을 고정 — 우리 리더가 0이 아닌 어떤 default 레이블도 받아 무시함(테스트); A6의 방아쇠에 셋째 값 오라클을 이름 붙임 |
