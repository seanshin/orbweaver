...
> 버전 0.2 · 2026-08-12 · **초안, Phase 0 결과에 따라 변경 가능**
...
**제외 (v1)** — CORBA Component Model, 실시간 CORBA 스케줄링, 기존 시스템의 비즈니스 로직 재작성, TCP 이외 프로토콜 위의 GIOP.
...
| 02 | `orbweaver-giop` | GIOP 1.0–1.2 메시지 프레이밍, TCP 위 IIOP, IOR 파싱·생성, 연결 관리 |
...
Phase 0에서 함께 진행 — **골든 IDL 코퍼스 v0** 구축. 중첩 struct, union, sequence, typedef, 상속, exception, valuetype, `oneway`, `any`를 망라한 20~30개 케이스. 이것 없이는 AI 품질을 아예 측정할 수 없다.
...
- `orbweaver-giop`: GIOP 1.0–1.2 프레이밍, `Request`/`Reply`/`LocateRequest`/`CancelRequest`, TCP 위 IIOP
- IOR 파싱·생성, 프로파일 처리, endpoint 재작성
...
- `orbweaver-dynamic`: DII/DSI/DynAny 동등 기능, JSON ↔ `any` 무손실 변환
- `orbweaver-forge`: S1–S5 — 흡수, 합성, 의미부착, **자가수정 루프를 갖춘 검증 게이트**
- SIDL 어휘 v1 확정
- 시맨틱 카탈로그: 임베딩 인덱스, 자연어 인터페이스 검색
- `orbweaver-mcp`: 레지스트리를 MCP 도구로 투영
- `orbweaver-guard` v1: 인가, dry-run, 감사로그
