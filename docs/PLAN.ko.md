...
> 버전 0.5 · 2026-08-12 · **Phase 0 완료(GO), Phase 1 배치 1–4 반영** — [`PHASE0.md`](PHASE0.md), [`PHASE1.md`](PHASE1.md) 참조
...
약 58주. ORB 코어 자체 구현이 기존 ORB 채택 대비 약 15주를 추가하며 그 대가로 완전한
MIT 자유도를 얻는다. 객체 모델과 신원 전파가 다시 13주를 더한다(§4.7, §4.8).

두 번째 증가는 조용히 흡수하지 않고 분명히 말해 둘 값어치가 있다. 이전 계획이 없는
셈 쳤던 두 가지를 산다. 일회성 호출만 하는 대신 단계를 넘어 참조를 쥘 수 있는
에이전트, 그리고 언제나 브릿지만 보는 대신 *누가* 호출하는지 아는 대상이다.
...
### Phase 2 — IDL 컴파일러·레지스트리·객체 모델 (11주)
...
- `orbweaver-object`: 값으로서의 참조(인라인 마샬링), 상속 그래프에서 로컬로 답하는
  `_is_a`, `_is_equivalent`/`_hash`, 객체 id와 활성화 정책, 서번트 관리자, 그리고
  현재는 따라갈 수만 있고 발행하지 못하는 `LOCATION_FORWARD` (§4.7)
...
작지만 순서가 의도적이다. MCP 브릿지 **다음이 아니라 함께** 나와야 한다. 에이전트
가시 참조를 원시 IOR로 내보내고 나중에 조이면, 그 사이에 가드를 우회할 수 있는 창이
생기고 이미 발급된 핸들을 회수해야 한다.

- `orbweaver-capability`: 불투명 핸들 발급·해석·범위지정·만료
- AnyJSON 객체 참조 인코딩(§4.5). 설계상 원시 IOR 형태가 없다
- 가드 통합: 모든 핸들 해석이 인가 지점이다
...
### Phase 5 — 신원과 자격증명 전파 (8주)

운영화의 한 줄이 아니라 자체 단계로 둔다. 감사 가능한 브릿지와 혼동된 대리자를
가르는 지점이고(R13), 위임을 잘못하면 버그가 아니라 권한 상승이기 때문이다.

- `TAG_CSI_SEC_MECH_LIST` 파싱, 피어별 능력 탐지
- `SecurityAttributeService` 컨텍스트(ServiceId 15): `EstablishContext`, 신원·인가 토큰
- GSSUP, `ITTPrincipalName`·`ITTX509CertChain`·`ITTAnonymous`
- OAuth2/JWT ↔ CSIv2 토큰 교환. 신뢰 경계를 문서화하고 모든 단언을 기록
- 위임 정책: 기본 거부, 인터페이스 단위, 결정으로 기록
- 토큰 만료 시 연결 중 재수립 (R17)
- 자격증명 위생: 기록 금지, 복구가능 저장 금지, 구조적 진단 제외 (R16)
- 강제할 수 없는 대상의 카탈로그 표시. "브릿지가 유일한 강제 지점"임이 가정이 아니라
  눈에 보이게 한다
