...
**S4가 게이트하지 않는 것: 재현성.** 같은 요구사항에 같은 프롬프트로 S1–S3를 두 번 돌리면 서로 다른 계약이 나오고 두 계약 모두 모든 게이트를 통과한다 — 이름이 다르고, 파라미터 타입이 다르며, 요구사항이 문자 그대로 적은 인가 스코프가 다른 값으로 바뀌었다([`pipeline-runs/2026-08-14-end-to-end.md`](pipeline-runs/2026-08-14-end-to-end.md), 근본원인 A). S2가 무엇을 고를 수 있는가, 그리고 재생성이 이미 등록된 계약에 무엇을 빚지는가는 방침 문제이며 [`decisions/D005-contract-stability.md`](decisions/D005-contract-stability.md)로 열려 있다(**제안됨**).
