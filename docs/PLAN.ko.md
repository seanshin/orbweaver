...
**계획상의 승격 조건** — 일 1,000회 이상 호출, 스키마 30일 무변경, 회귀 스위트 통과, 파괴적 스키마 변경 시 자동 강등. **`PromotionPolicy`가 구현한 것은 그보다 좁다**(2026-08-25 확인): 호출 횟수(`min_calls` — 비율이 아니라 절대 수)와 실패율 상한, 그리고 게이트는 신원 검사(§7.4의 I4). 이 모듈은 시계를 읽지 않으며 `demote`는 워크스페이스 어디에도 없다. 달력 조항은 프로세스보다 오래 사는 저장소가 필요하고(§6, 아직 향후 작업), 그래서 이 문단은 설계 의도인데 동작처럼 읽히고 있었다.
...
`ServantLocator`가 `LOCATION_FORWARD`를 만들어내며, 클라이언트가 따라가고 서버도 이제
발행한다 — `Dispatch::forward`와 `Dispatch::redirect`, 임시와 영구, 1.2 → 1.0/1.1 상태
강등 포함(`orbweaver-giop::server`, 2026-08-12·2026-08-19 착지; 이 문장은 그 절반이
착지한 뒤로 열사흘 동안 *발행하지 못한다*고 말하고 있었다). 동적으로 생성된 서비스는 사라진 서번트에 대한
...
원래 계획에서 미착수: **없다**. Phase 6(운영화)는 2026-08-14에 시작되었고 —
[`PHASE6.md`](PHASE6.md)에 배치 둘, R7 엔드포인트 재작성과 두 번째 호스트 프로브 —
나머지는 §7.3의 스트림 D다. 이 줄은 그 뒤로 열하루 동안 "Phase 6"라고 말했다. 바로 위
칸이 같은 이유로 착지한 세 항목의 나열을 그만둔 직후에도. 상태는
[`COMPONENTS.md`](COMPONENTS.md)에 살고, 이 줄은 손대지 않은 단계가 없다는 것만
말한다. 모델이 루프에 들어가는 S1–S3는 2026-08-13에
...
>
> 아래 스트림들과 **1차 완성** 사이에 남은 것 — 0순위 순서로 정렬한 열린 투명성
> 구멍들과, 각각이 무엇과 함께 착지하는지 — 는
> [`PLAN-FIRST-COMPLETION.md`](PLAN-FIRST-COMPLETION.md)에 있다 (2026-08-29).
> 이 문서의 스트림이 아니라 하네스가 실행 시점에 D029에서 읽어내는 구멍 셀에
> 대고 계획하므로, 여기의 한 절이 아니라 형제 문서다.
...
- **무엇:** ~~레지스트리에서 `orbweaver-gen` 스텁/스켈레톤(Rust 먼저, 다음 Python)~~
  (둘 다 착지; Python 클라이언트 타깃은 2026-08-14, `0d6ef7e`);
  ~~승격 엔진(회귀 게이트를 갖춘 동적 → 정적)~~(2026-08-13 착지, `99abbfa`; 게이트는
  §7.4의 I4); 주석에서 계약 테스트 생성(**남은 유일한 항목** — §11의 지향 A8).
  이 셋은 §7.2가 2026-08-19에 나열을 그만둔 바로 그 세 항목이다. 수리는 그곳에만
  적용되고 한 절 뒤인 이곳에는 적용되지 않아, 둘은 엿새를 더 지워지지 않은 채
  서 있었다;
