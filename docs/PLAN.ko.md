...
> 버전 0.6 · 2026-08-13 · **Phase 0–3.5 완료, Phase 5 절반 착지** — [`PHASE0.md`](PHASE0.md) · [`PHASE1.md`](PHASE1.md) · [`PHASE2.md`](PHASE2.md) · [`PHASE3.md`](PHASE3.md) · [`PHASE5.md`](PHASE5.md)
...
원래 계획에서 미착수: Phase 4(정적 생성·승격), Phase 6(운영화), 그리고 모델이
루프에 들어가는 S1–S3.
...
| 지점 | 스트림 | 참이어야 하는 것 |
|---|---|---|
| **I1. 생성된 스텁도 가드를 지난다** | B × C | 정적 스텁이 동적 호출과 같은 `Exposure`/`Delegation`/감사 경로를 지난다. 가드를 우회하는 스텁은 §4.7의 우회를 컴파일된 형태로 재현한다. 정적 클라이언트에 대해 전사 유출 테스트를 돌려 검사. |
| **I2. 파이프라인 산출물은 안전하게 노출된다** | A × D | S5 등록은 **노출 꺼짐**으로 카탈로그에 들어간다. 생성된 인터페이스가 에이전트에 보이려면 손으로 쓴 것과 동일한 명시적 허용목록이 필요하다. |
| **I3. 검색이 주석을 세탁하지 않는다** | A × D | 임베딩 인덱스는 `ai_desc`를 데이터로 다룬다(R11). negative 말뭉치의 주입 케이스가 동결 질의 벤치마크에 포함된다. |
| **I4. 승격은 신원을 보존한다** | B × C | 승격된 정적 경로는 그것이 대체한 동적 경로와 동일한 `Caller` 단언 동작을 가진다. "승격이 신원 컨텍스트를 떨어뜨렸다"가 회귀 게이트의 첫 테스트다. |
