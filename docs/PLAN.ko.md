...
| 상호운용 매트릭스 통과율 | — | 100% |
| 동적 경로 오버헤드 (정적 스텁 대비) — `call-bench` p50 비율, 루프백, 워밍업 300쌍 후 형태별·경로별 n ≥ 2000 | 가장 넓은 형태(`echo_many`, 24 B 문자열 64개)에서 1.06~1.07×, 12코어 랩톱 부하 ~3~4, 2026-08-18 | **모든** 형태에서 ≤ 1.5×; 절대 지연 조항 없음 |
