...
| **I4. 승격은 신원을 보존한다** ◐ | B × C | 승격된 정적 경로는 그것이 대체한 동적 경로와 동일한 `Caller` 단언 동작을 가진다. "승격이 신원 컨텍스트를 떨어뜨렸다"가 회귀 게이트의 첫 테스트다. **게이트 착지**: `verify_promotion`이 결과 비교 전에 `IdentityDropped`를 검사하며 가드 페이크로 단위 검증됨. 라이브 절반(실제 승격의 두 감사 로그를 피어에 대고 게이트에 통과)은 미배선이고 모듈에 그렇게 적혀 있다. |
