...
|---|---|---|
| **Location** | where the target runs | **STANDING: named floor**. **measured, with a known leak**: `LOCATION_FORWARD` and `_PERM` are served and followed, and R7 rewrites an IOR for a dialable address — but `Connection::move_to` restored a hand-written field list and dropped two configured limits across every forward until today, so the *caller's* limits changed when the object moved. Fixed; the class is the leak to watch. **A second instance, found 2026-08-26 and ACCEPTED AS A NAMED FLOOR on 2026-08-31**: `moe::Router::select` returns `ExpertSeq` — N object references, each an `Ior` stored verbatim from `register_expert` and marshalled inline with host, port and object key. A caller learns where every candidate expert runs, which is exactly what this row says it must not be able to tell. [`D037`](D037-what-a-selection-hands-back.md), option C — read its status there. **The floor is asserted rather than described**, which is what separates it from silence: `expert_service.rs`'s `a_selection_hands_the_caller_every_candidate_address` fails if the addresses stop being there, so the day somebody proxies them this row moves *deliberately*; its control is a `select` that hands back references with no profile, and the test goes red. `corpus/golden/22`'s comment beside the operation is now a statement of the accepted limit rather than an observation about a hazard, and §4.7's rule is the authority half of the same fact. **The reason is a reason and not an intention**: closing the row needs `Router::dispatch` to be the only way through, refused on separate grounds (D006 option E) that D037 does not reopen, and the candidate that narrows the exposure buys displacement, which D035 has already ruled is not closure. **What is not the question** (D037 §2): whether a caller may know *which* experts exist — it may, `Router` being a control-plane gate, and a second test says so beside the first so the floor is not misread as *a caller should learn nothing*. What does not carry across is addresses. **The cost is accepted with it**: D037 §6.4 says a criterion whose rows are mostly named floors is measuring the shape of the repository rather than the transparency, and this is the second such floor after D035's lifecycle one. **A third thing under this row, closed 2026-08-26 and not one of the two leaks above**: the probe path *lied*. See the `LocateRequest` subsection below for what moved and why the row's status sentence is unchanged. |
| **Backend** | what implements it | **STANDING: open leak**. Mostly held, and the "mostly" has **one** open leak. **Corrected 2026-08-28**: this cell used to open by naming two, the first being `spike_experts`' server root key colliding with its derived registry key. That collision was fixed in `bd88d36` at 15:54 on 2026-08-26 — **two and a half hours before the commit that wrote this cell at 18:21** — so the sentence was false when it was written, and `CHANGELOG.md`, `D028` §143 and `COMPONENTS.md` had all already recorded the fix. One `MOE_BASE_KEY`, one `plane()` building server and servant together, and a gate that asks the property (*the server's identity is not one of the faces it serves*) rather than the spelling. It is named here rather than deleted because a row that quietly loses a leak is indistinguishable from one that never had it. The remaining leak was found by a hand-written C peer dialling us on 2026-08-26 and is gated but not closed:** *a servant is behind a POA and a reference* is true only of servants whose `knows` reads the object key it is given. For a servant that answers `true` without reading it the key selects nothing, so what is behind the endpoint is not a reference — the address is the only thing naming a target. (**Said as *inheriting a default* until 2026-08-31**, which is how it was spelled when the C peer found it; D036 deleted that default on 2026-08-29 and the leak survived the deletion unchanged, because writing `true` and inheriting `true` answer a caller identically.) A caller establishes it in one call: fabricate any key at that endpoint and be answered, where CORBA 3.4 §15.3.8.6's own default (`USE_ACTIVE_OBJECT_MAP_ONLY`) would have said `OBJECT_NOT_EXIST` and told it nothing. What it learns is a backend fact — this endpoint holds one undifferentiated servant rather than a POA with an active object map — which is what this row says it must not be able to tell. Same family as the collision above: object-key identity not enforced. The question of what to do about it was asked in [`D036`](D036-what-a-servant-answers-for-a-key-nobody-activated.md) and answered — option A, approved 2026-08-29, landed in `81cc546`. The argument is `orbweaver_giop::server::Dispatch::knows`, the measurement is `crates/orbweaver-giop/tests/a_key_nobody_activated.rs`. **Rewritten 2026-08-31, and everything this cell used to say about the leak's shape had been false for two days.** It said the leak was *inheriting a permissive default*, counted `26 of the workspace's 72` implementations as inheritors, named `spike-server` as one of them, and named `spikes/e2e/servant.rs` and `spikes/estate/servant.rs` as checking the key in `dispatch_body`. D036 deleted the default, so **inheriting does not compile and that count is of an empty set**; `spike_server.rs` answers `object_key == ECHO_KEY`; and both spikes moved their check into `knows` in the same batch. The harness reads this cell at run time and printed the dead figure on every run in between — *a figure in prose carries the date it was measured, or comes from a script that writes it.* **What the leak is now, and it is the same leak.** D036 closed nothing and says so: a servant that *writes* `true` answers for every key exactly as one that inherited it did. What changed is that the answer cannot be given by omission, which makes the population nameable — *a `knows` whose body never reads the key* — and `a_key_nobody_activated.rs` computes it from the tree rather than quoting it here. **Today it is 20 of 82 implementations, and 0 of those 20 are in code a build emits** (all 20 sit inside `#[cfg(test)]` or under a `tests/` target; measured 2026-08-31, and printed by that test under `--nocapture` rather than maintained in this sentence). The reachable half being empty is now an assertion rather than an observation: the first servant compiled into a binary or a library that answers for every key fails by name and line, and so does one that answers `true` while checking the key on the request path — the probe/request disagreement, whose hunt had gone vacuous when D036 emptied the set it was scoped to. **The row does not close, and one of the two reasons has since been measured rather than described.** (1) The scan saw an *unconditional* `true` and could not see a `knows` that consults something and still answers for a superset. **Half of that was closed by looking, 2026-08-31**: the roster now also asks whether `knows` has a *branch* whose whole answer is `true`, and the tree holds **22 such servants, exactly 1 of which a build emits** — `orbweaver_gen::seam::ForeignServant`, whose `knows` reads `match &self.identity { Some(i) => …, None => true }`, so one built without a home answers for every key a caller can fabricate. That is this row's leak with a deployable servant behind it, it is pinned at one by name, and what to do about it is asked in [`D039`](D039-what-a-servant-with-no-home-answers-for.md) rather than settled in code — a `ForeignServant` usually exists before its server binds, so an identity cannot simply be required. **The bound that remains is narrower and in the same direction**: a `knows` answering from a set merely *wider* than its servant's holds no `true` literal at all and is invisible to any of this. (2) Every one of the 21 fixtures is a servant that would leak if anything ever served it. What can be said is narrower and is what the assertion says: **no servant this workspace builds answers for a key nobody activated.** |
| **Language** | what it is written in | **STANDING: held**. **The construction leak is closed, and so are both of the narrower ones §6.1.1 called *the ones worth closing next***: item 5 on 2026-08-26 and item 4 on 2026-08-31. **This opening read *"three narrower ones remain (2026-08-26)"* until 2026-08-31**, a typed and dated count that its own §6.1.1 table already contradicted — the three it was counting are that table's items 1–3, which the table's own header calls *differences in what a servant author can get wrong, not in what a correct servant answers*, each ending in a legal CORBA refusal. A count of author-error cases is not a count of leaks, and a cell that grows by appending corrections leaves the oldest claim in the position a reader takes as the answer. That is what the `STANDING:` token above exists to stop — and it is spelled `STANDING` rather than `STATUS` because `spikes/decision_status.py` owns the latter for a decision's own state, and reading five transparency rows as five decision markers is how the first draft of this token made this file claim two states at once. The gate caught it, which is the fourth time it has caught decision vocabulary being borrowed for something that is not a decision. A Python servant is dispatched into by `orbweaver_gen::pyservant`, and `tests/python_servant.rs` compares one against the generated Rust servant for the same contract — 19 calls × 3 GIOP versions × 2 byte orders, **byte-identical replies**, with a negative control that perturbs five answers and asserts each is seen. **The peer half is now measured in both byte orders (2026-08-26):** omniORB's client little-endian, and JacORB's big-endian — the order taken from §15.4.1's flag byte on every request rather than from the peer's language — with the Python servant's 11 replies byte-identical to a Rust servant's for the same driver run, at IIOP 1.2 and 1.1. What remains is listed in §6.1.1 and none of it is the old *"cannot be a target at all"*. **Later the same day the seam stopped being one language's and a foreign servant stopped being a singleton leaf** — §6.1.1's item 5 is closed and its reason was wrong as written; the seam is `orbweaver_gen::seam`, its protocol is a value both implementations publish and are asserted against (`tests/the_seam_is_one_protocol.rs`), and a foreign servant is told which object it is, answers for its own keys only, and can hand out a reference to a neighbour — 126 cells byte-identical against the generated Rust skeleton, four controls each run red. **What was left under this row was one thing and it is CLOSED (2026-08-31, D038 option A)**: a reference *arriving* used to be a handle the far side could pass back and not use, because invoking it needs a call travelling the other way and the protocol had no message for one. It has one now — the far side sends `{"invoke": …}` naming a handle, this side dials, and the answer arrives before the reply to the call the servant is still inside, so the seam is **re-entrant**. Measured by `crates/orbweaver-gen/tests/a_call_travelling_the_other_way.rs`: a Python servant handed a reference to a Rust target on a real socket invokes it, the target records exactly one call, and the servant refuses unless the value it read back is the one that target answers — **neither assertion sufficient alone**, which is why both are there. Its control is the same servant with the nested call removed, and the target is never reached. Three invariants held and are why this closes the row rather than moving it: the far side never learns an address (it names a handle), the handle table stays the boundary (a handle nobody issued resolves to nothing), and a refused nested call is a well-formed answer rather than an `Err`, so a foreign servant can catch one and go on answering. **What this does not establish**: it is not a side-by-side comparison of a Rust and a Python servant doing the same thing — what it establishes is that the Python one *can*, which is the half that was missing. And the nested call's result reaches the far side in AnyJSON rather than as a mapped value, because that side knows only the repository id the reference advertises; the marshalling is done where the contract is known. |
| **Activation / load** | whether it is loaded right now | **STANDING: named floor**. **Closed at the POA 2026-08-26, and the row's own diagnosis was wrong.** This row named `moe::Router::select` as the leak. `select` is a **contract, not a leak**, and the argument is from corpus/golden/22 rather than from convenience: `Constraints` declares no residency member, so filtering would apply a constraint nobody expressed; the contract already gives load state two homes where it is a *value a caller asks for* (`ExpertLoader::status`, `//@ ai_effect: read_only` with no `ai_authz`, and `Capability::state` through `Expert::describe`); `Router` being `//@ ai_desc: Control-plane gate` settles that its callers **may know**, which is a right to be *told* and not a licence for a side channel; and — decisively — **a filter could not have closed it.** `select` answers at T and the caller dials at T+ε, so an expert RESIDENT at T can be evicted before the dial, and a reference obtained from `Expert::delegate`, from an earlier call or from a string never went through the operation at all. A filter changes how *often* a caller can tell, never *whether*. The reason the code gave for the omission was separately false and is corrected: *"the caller's cue to `prefetch`"* names an action the cued caller generally cannot take — `prefetch` is `oneway`, it lives on `ExpertLoader`, an object `Router` never hands over, and it is gated `//@ ai_authz: moe.loader.residency`, which `moe.router.select` does not imply. **The leak was one layer down, in what the *reference* does across an eviction**: `residency::MissPolicy`'s two variants both answered `Located::Unknown` for an OFFLOADED expert, the POA turned that into `OBJECT_NOT_EXIST`, and the same reference invoked twice by the same caller answered differently. `MissPolicy::Activate` closes it — demand-load inside `locate`, answer `Located::Here` — and being a POA-level fact it holds for *any* target, which is what this criterion asks and what a fix inside one application contract could never have given. The refusal that had ruled demand loading out is kept verbatim and quoted in `MissPolicy`'s rustdoc: every clause of it is about **cost**, and priority zero ranks a cost below a leak. **What is not closed, and is not to be looked for elsewhere:** (1) **time** — a demand-loaded call is slower and a caller with a clock can tell; in one process the only alternatives are forwarding to a node where the target is loaded (there is no second node) or making every call as slow as the slowest, and in this repository a load is two map writes so the latency is not measurable here either; (2) ~~**nothing in the tree mounts `ExpertLocator` on a served POA**, so which variant a deployment runs is undecided and a deployment on either refusing variant still leaks~~ — **superseded later the same day, and the row does not move.** `crates/orbweaver-object/src/expert_host.rs` is the mount: a servant that owns a `Poa` (Persistent, `AskLocator`) *and* the `ExpertLoader` for the ids that POA mints keys for, serving `moe::Expert`'s three declared operations, defaulting to `MissPolicy::Activate`. The ownership question that had been left open — *who owns an expert's server* — is answered there and not restated here: **whoever owns the expert's residency owns its server, and the mount is the statement that these are one object**, which is why neither existing servant could be it (`ExpertService` owns the loader and stores registrants' `Ior`s verbatim, so its experts live elsewhere; `TenantService` serves experts and has no residency, and giving it one would leave the tenantless shared base with no owner for its own). Nothing is wired to it, so **no existing deployment behaves differently** — which is also what is still open here: the mount is *available and not taken*, the four `spike_*` binaries and both services construct none, and an expert registered from another process is that process's to host. Two further things are named rather than left to be met: a `LocateRequest` probe is a second surface for this property, so `ExpertHost::knows` is residency-independent (measured, with a control that reads residency and reproduces the probe leak); and `moe::Capability`'s fifth member is `Residency state`, so the contract's own `describe()` **reports** load state, which is this row's own "a value a caller asks for" and not a leak — its reply is unchanged across an eviction for an *ordering* reason (the demand load runs inside `locate`, before the servant) rather than because nothing in it depends on residency. (4) **A demand load that fails is `OBJECT_NOT_EXIST` — the leak put straight back** — and it is the *same open question* as `MissPolicy`'s deliberately-absent deadline: both want a reply that is neither "here" nor "never existed", the candidates are `TRANSIENT` or a `Located` variant carrying a retry-after, and neither has a client that would act on one. Unreachable in this repository (both edges the `Activate` arm takes are unguarded), live in a deployment where the copy can fail. (5) `Router::dispatch` is still refused (D006 option E, its reason false as written — see D006's 2026-08-26 amendment) and is still not the operation that would have closed this. |
| **Lifecycle stability** | that the above survives add / remove / move / load / evict at runtime | **STANDING: named floor**. **Measured 2026-08-27, over a floor that is named rather than closed.** X was answered and the answer was not X: D035 asked which of *displacement* and *closure* moving the leak from N addresses to one is claiming, and the owner answered **displacement is not closure** — the same answer the event-channel subsection below already gives for the bootstrap. So the row stopped waiting on a decision that could not reach zero. **The floor:** a caller of a removed target can tell it is gone, because it must be given one address to send a first packet to, and nothing inside one process changes that. **Above the floor, measured:** removing one target is invisible to a caller of another — `crates/orbweaver-giop/tests/what_a_caller_can_tell_about_a_removal.rs`, with the floor asserted rather than left to prose so a change that made it stop being true could not pass unnoticed, a counted anti-vacuity companion proving the two targets could be told apart while alive, and the control `ORBWEAVER_LEAK_CONTROL=removal_isolation`, run red. **The second floor is measured now (2026-08-29), and it is still a floor.** `Orb::shutdown` says §9.4.10's goodbye; a target killed under the same caller says nothing at all. `spikes/orb_shutdown.sh` drives both arms from one fixture — the graceful one calls `Orb::shutdown` on the servant's entry signal, the other SIGKILLs itself at that same instant with the servant still held — and the peer, which imports no ORB, is told which to expect. The measurement is the **2×2**, not the two matching runs: each fixture is also driven against the *other* expectation and required to be refuted, because a run where the peer expects what it gets proves nothing on its own. Killed: one `reset`, no reply, no goodbye. Stopped: a reply to request 1, then `CloseConnection`. Crossed: both refuted. What is asserted is the **absence of the goodbye**, not the presence of a reset — whether an abruptly dead process is answered with an RST or a FIN is the platform's business, recorded rather than pinned. Measuring it does not move this row: a caller *can* tell, which is what a floor means. What changes is that the floor can no longer stop being true without something going red. O1 and D034 remain what measures the removal itself, from a peer's own socket. The serve-site shape was repaired 2026-09-04, by the rule rather than the count — *a server a fixture starts is stoppable by the route its own teardown uses*: **12 of this workspace's 85 serve sites pass `|| false` (2026-09-04), every one carries a recorded refusal at the site, and the unexplained count is 0.** Four are the stopping measurements' own contrast arms, where the stop under test is the ORB's flag and a predicate of the test's own would prove nothing; eight are fixtures whose documented teardown is the driver killing the process, with no in-process actor left to raise a stop. The other nine sites (six files) shared one root cause — a helper that spawns a detached serve thread and discards every stop handle — and took one fix: the helper keeps the server's own `StopFlag` (D034's route) and its owner's teardown raises and joins it. The figure was **17 of 63** when it was first written on 2026-08-27 and **nothing computed it** — it was typed once and quoted since, which is *a floor is not a figure* in its plainest form: both halves drifted and no gate could go red, because there was no gate; it read **21 of 80** when `spikes/serve_sites.py` became the method on 2026-08-30. That script keeps the count, tells a recorded refusal (a `serve_sites: refusal` comment at the site, carrying its reason) from an unexplained `|| false`, and the count stays a **lower bound**: it classifies spellings, so a stop handed in through a binding reads as stoppable. The next unexplained site is a defect, not a figure. |
...
이는 위치 행이 알 수 없어야 한다고 적은 바로 그것이다.

**2026-08-31 — 이름 붙인 바닥으로 받아들였다(D037 선택지 C; 상태는 그 문서에서
읽는다).** 침묵과 갈리는 지점은 **바닥을 서술하지 않고 주장한다**는 것이다:
`a_selection_hands_the_caller_every_candidate_address`는 주소가 사라지면 실패하므로,
누군가 이 참조들을 프록시하는 날 행은 조용히가 아니라 **의도적으로** 움직인다. 그
대조군은 프로필 없는 참조를 돌려주는 `select`이고, 테스트는 빨개진다.
`corpus/golden/22`의 주석은 이제 위험에 대한 관찰이 아니라 **받아들인 한계의
진술**이다.

**이유는 의도가 아니라 이유다**: 닫으려면 `Router::dispatch`가 유일한 경로여야
하는데 그것은 별개 근거(D006 선택지 E)로 거절되어 있고 D037은 그것을 다시 열지
않으며, 노출을 좁히는 후보는 D035가 이미 폐쇄가 아니라고 판정한 **변위**를 살 뿐이다.
**질문이 아닌 것**(D037 §2): 호출자가 *어떤* 전문가가 있는지 알아도 되는가 — 알아도
된다. `Router`는 제어 평면 게이트이고, 그래서 첫 테스트 옆에 그것을 말하는 두 번째
테스트가 있다. 건너가지 않는 것은 **주소**다. **비용도 함께 받아들인다**: 행 대부분이
이름 붙인 바닥인 기준은 투명성이 아니라 저장소의 모양을 재는 것이며(D037 §6.4),
이것은 D035의 생애주기 바닥에 이은 두 번째다.

**2026-08-28 정정 — 이 행이 이름 붙였던 구멍 둘 중 하나는 쓰일 때 이미
닫혀 있었다.** `spike_experts`의 키 충돌은 2026-08-26 15:54의 `bd88d36`이 고쳤고,
이 셀을 쓴 커밋은 같은 날 **18:21**이다 — 두 시간 반 뒤다. CHANGELOG·D028·COMPONENTS
셋이 이미 수정을 기록하고 있었다. 지우지 않고 이름을 남긴 이유는, 구멍을 조용히
잃은 행은 애초에 없던 행과 구별되지 않기 때문이다. 아래의 `PyServant` 언급도
같은 부류다: seam 리팩터가 그 서번트를 `knows`를 재정의하는
`crate::seam::ForeignServant`로 옮긴 뒤였고, 그래서 명단은 이제 트리에서 계산된다.

**2026-08-26 측정 — 백엔드 행에 두 번째 구멍이 이름을 얻었다. 막지 않고 고정만
했다.** 손으로 쓴 C 피어가 우리에게 걸어 보고 찾았다. *서번트는 POA와 참조 뒤에
있다*는 문장은 `knows`가 주어진 객체 키를 **읽는** 서번트에 대해서만 참이다. 키를
읽지 않고 `true`를 답하는 서번트에서는 객체 키가 아무것도 고르지 않으므로, 종단점
뒤에 있는 것은 참조가 아니라 **주소**뿐이다. (**2026-08-31까지는 이것을 *기본값
상속*이라고 적었다** — C 피어가 찾았을 때의 철자다. D036이 2026-08-29에 그 기본값을
지웠고 구멍은 그대로 살아남았다. `true`를 쓰는 것과 물려받는 것은 호출자에게
똑같이 답하기 때문이다.) 호출자는 한
...
적은 바로 그것이다. 위의 키 충돌(이제 닫힘)과 같은 계열이며, 원인은 객체 키의 정체성이
강제되지 않는다는 것이다. 논거의 집은 `orbweaver_giop::server::Dispatch::knows`,
측정은 `crates/orbweaver-giop/tests/a_key_nobody_activated.rs`. 무엇을 할 것인가는
[`D036`](D036-what-a-servant-answers-for-a-key-nobody-activated.md)에서 물었고
답해졌다 — 선택지 A, 2026-08-29 승인, `81cc546`에 착지.

**2026-08-31 재작성. 이 셀이 구멍의 모양에 대해 적어 두었던 것은 이틀 동안 전부
거짓이었다.** 구멍을 *관대한 기본값을 물려받는 것*이라 적었고, 구현 `72개 중 26개`를
상속자로 셌고, `spike-server`를 그중 하나로 지목했고, `spikes/e2e/servant.rs`와
`spikes/estate/servant.rs`가 `dispatch_body`에서 키를 검사한다고 적었다. D036이
기본값을 삭제했으므로 **상속은 컴파일되지 않고 그 수는 공집합의 수**이며,
`spike_server.rs`는 `object_key == ECHO_KEY`를 답하고, 두 스파이크는 같은 배치에서
검사를 `knows`로 옮겼다. **하네스는 이 셀을 실행 시점에 읽는다** — 그래서 죽은
수치가 그 사이의 모든 실행에 찍혔다. *산문의 수치는 측정 날짜를 달거나, 그것을 쓰는
스크립트에서 온다.*

**지금 구멍이 무엇인가, 그리고 그것은 같은 구멍이다.** D036은 아무것도 닫지
않았고 스스로 그렇게 적는다: `true`를 **쓴** 서번트는 물려받은 서번트와 똑같이 모든
키에 답한다. 바뀐 것은 답을 **누락으로 줄 수 없다**는 것이고, 그래서 모집단에 이름을
붙일 수 있게 되었다 — *키를 읽지 않는 `knows` 본문*. `a_key_nobody_activated.rs`가
그것을 트리에서 계산하며 여기에 인용하지 않는다. **오늘 구현 82개 중 20개이고, 그
20개 중 빌드가 내보내는 것은 0개다**(전부 `#[cfg(test)]` 안이거나 `tests/` 타깃
아래. 2026-08-31 측정, 수치는 그 테스트가 `--nocapture`로 찍는다).

닿을 수 있는 절반이 비어 있다는 것은 이제 관측이 아니라 **단언**이다: 바이너리나
라이브러리에 컴파일되면서 모든 키에 답하는 첫 서번트가 이름과 줄 번호로 실패하고,
`true`를 답하면서 요청 경로에서 키를 검사하는 서번트도 그렇다 — D036이 그 사냥이
겨냥하던 집합을 비웠을 때 공허해졌던 요청/탐침 불일치다.

**행은 닫히지 않으며, 초록인 실행이 실제보다 크게 읽히지 않도록 두 가지 이유를
여기 적는다**: 스캔은 *무조건적인* `true`를 볼 뿐, 무언가를 참조하면서도 자기 서번트가
가진 키의 상위집합에 답하는 `knows`는 보지 못한다. 그리고 그 20개 픽스처는 전부,
무엇인가 그것을 서비스하는 순간 누출할 서번트다. 말할 수 있는 것은 더 좁고, 단언이
말하는 바로 그것이다: **이 워크스페이스가 빌드하는 어떤 서번트도 아무도 활성화하지
않은 키에 답하지 않는다.**
...
러스트독에 있으며 어느 쪽도 다시 적지 않는다. 같은 날 측정했던 serve 지점의
모양은 **2026-09-04에 수리되었다** — 수가 아니라 규칙(*픽스처가 띄운 서버는 그
픽스처 자신의 해체 경로로 멈출 수 있어야 한다*)에 맞춰서다. **이 워크스페이스의
serve 지점 85개 중 12개가 `|| false`를 넘기고(2026-09-04), 열두 곳 모두 그 자리에
거절이 기록되어 있으며, 설명 없는 곳은 0이다.** 넷은 멈춤을 재는 측정 자신의 대조
팔이다 — 시험 대상인 멈춤이 ORB의 깃발이므로, 테스트 자신의 술어는 증명을 무효로
만든다; 여덟은 문서화된 해체 경로가 드라이버의 프로세스 kill인 픽스처다 —
멈춤을 올릴 프로세스 내 행위자가 남아 있지 않다. 나머지 아홉 지점(여섯 파일)은
근본원인 하나를 공유했다 — 도우미가 분리된 serve 스레드를 띄우고 멈출 손잡이를
전부 버린다 — 그래서 수정도 하나였다: 도우미가 서버 자신의 `StopFlag`(D034의
경로)를 쥐고, 소유자의 해체가 그것을 올리고 join한다. 처음 적힌 수는 2026-08-27의
**63개 중 17개**였고 **아무것도 그것을 계산하지 않았다** — 한 번 타이핑되고 이후
인용되었을 뿐이며, 가장 평범한 형태의 *하한은 수치가 아니다*였다: 두 언어 반쪽이
모두 어긋났고 빨개질 게이트가 없었다; `spikes/serve_sites.py`가 방법이 된
2026-08-30에는 **80개 중 21개**로 읽혔다. 그 스크립트가 수를 계산하고, 기록된
거절(지점의 `serve_sites: refusal` 주석과 그 사유)과 설명 없는 `|| false`를
구별하며, 수는 여전히 **하한**이다 — 철자를 분류하므로 바인딩으로 건네진 멈춤은
멈출 수 있는 것으로 읽힌다. 다음 설명 없는 지점은 수치가 아니라 결함이다.

**2026-08-27 — X가 답해졌고 답은 X가 아니었다.** D035가 D029가 요구한 질문에
답했다: 구멍을 주소 N개에서 하나로 옮기는 것이 *폐쇄*인가 *전가*인가 — 소유자는
**전가는 폐쇄가 아니다**라고 답했으며, 이는 아래 이벤트 채널 절이 부트스트랩에 대해
이미 내놓은 답과 같다. 그래서 이 행은 **0에 닿을 수 없는 결정을 기다리기를
그만두었다.** **바닥**: 제거된 대상의 호출자는 그것이 사라졌음을 알 수 있다 —
호출자는 첫 패킷을 보낼 주소 하나를 받아야 하고, 한 프로세스 안에서는 그것을 바꿀
수 없다. **바닥 위에서 측정된 것**: 한 대상의 제거는 다른 대상의 호출자에게 보이지
않는다(`crates/orbweaver-giop/tests/what_a_caller_can_tell_about_a_removal.rs`).
바닥은 산문이 아니라 테스트로 단언되어 있어, 그것이 참이 아니게 되는 변경이 조용히
지나갈 수 없다. 그리고 살아 있는 동안 두 대상을 구별할 수 있었음을 증명하는 계수된
동반자와, 붉게 돌려 본 대조군 `ORBWEAVER_LEAK_CONTROL=removal_isolation`이 있다 —
*"알 수 없다"*는 아무 일도 일어나지 않는 세계에서도 통과하기 때문이다. **여전히
측정되지 않은 것**: 멈춘 것이 아니라 **죽은** 대상. `Orb::shutdown`은 §9.4.10의
작별을 말하고 죽은 프로세스는 리셋을 남기며, 호출자는 그 둘을 구별할 수 있다.
그것은 두 번째 바닥이며 여기에 이름을 붙여 둔다.
...
| 4 | ~~**An object reference argument reaches a Python servant as an opaque handle it cannot invoke**~~ — **closed 2026-08-31 (D038 option A)**. §4.5 still emits no IOR and a reference still crosses as a token into the bridge's table, which is §4.7 and does not change; what changed is that the token can now be *invoked* by asking this side to dial it. The row is kept rather than deleted because a row that quietly loses a leak is indistinguishable from one that never had it | it was: on any contract passing a reference the servant must *use*, the Python servant could not and the Rust one could. Measured closed by `a_call_travelling_the_other_way.rs` |
...
**Item 4 stood, and was the whole of what this table said. It closed on
2026-08-31** under [`D038`](D038-a-call-travelling-the-other-way.md) option A.
A reference *arriving* was a handle the far side could not invoke — the inbound
half, and a different kind of work: it needed a call travelling the *other* way
through the seam, which the protocol had no message for. Naming it that way
rather than as *"references do not work"* is what made the remaining work one
message rather than a rewrite, and is the point of having re-measured.

The protocol has that message now (`invoke`, answered as `answer`, version 2),
which makes the seam **re-entrant**: the far side asks mid-answer, and this side
dials and answers before the reply to the call it is still inside. What the
sizing changed about the work is worth keeping — it was never *a second message
kind* on its own; the re-entrancy is the property, and D038 §2's three
consequences are why it was a decision rather than a patch.

*항목 4는 2026-08-31에 닫혔다(D038 선택지 A). 그것을 "참조가 동작하지 않는다"가
아니라 **도착한 참조를 호출할 수 없다**로 이름 붙인 것이 남은 일을 재작성이 아니라
메시지 하나로 만들었고, 그것이 다시 잰 이유다. 이제 seam은 **재진입**이다.*
