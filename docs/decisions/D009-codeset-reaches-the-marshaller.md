...
| # | Batch | Oracle |
|---|---|---|
| 1 ✅ | `TextCodec` + `Option<Arc<…>>` slot in `orbweaver-cdr`; `None` is today's behaviour | **byte identity**: the whole workspace green and every peer capture re-read unchanged. A refactor that moves one byte has failed — **held**, 1306 tests unchanged (`6faeea6`) |
| 2 ✅ | `codeset` implements it; `Connection`/`Mux` attach it; **the reply side (§7.1)**; §9.3.1.6's rule that a `wchar` inside an encapsulation is always the 1.2 form, in code rather than in a comment | the existing peer captures byte for byte; a negative control per rule; a request and its reply asserted to use one agreement — **held** (`e3aa7bf`). The §7.1 test caught its own author: it declared ISO-8859-1 and encoded the body as UTF-8, and the servant read `cafÃ©` |
| 3 ✅ | `Cdr for String`/`WString` ask the stream; `wide()` kept as the §9.3.1.6 encapsulation rule and `default_codec()` **not** deleted — see §7.3's correction, which this row cited before it was made | byte identity held (1327 unchanged) plus two new assertions: a `wstring` differs between 1.1 and 1.2, and an unattached stream still writes the encapsulation form. Negative control: putting the stub back on the constant fails the first (`774a0ed`) |
| 4 ✅ **BLOCKED** | ~~L2's seven publish sites~~ (**landed early**, `b9910f6` — a byte comparison between a generated skeleton and its hand-written servant went red the moment one published `TAG_CODE_SETS` and the other did not, so the cause landed whole rather than two sites at a time); **remaining**: a non-empty `char` conversion list | a peer advertising ISO-8859-1 **without** UTF-8. If none can be produced here, the list stays empty and the batch reports **BLOCKED** — advertising a conversion we cannot demonstrate is the defect this document exists to avoid. **Answered 2026-08-18: BLOCKED, and the answer is stronger than the condition asked for.** Eleven peer configurations probed and every one reaches UTF-8 — neither ORB exposes an option that names its conversion list. And offering the conversion was measured to *lower* what a working peer sends: JacORB native ISO-8859-1 moves down to it and truncates Korean to low octets **raising nothing**, while omniORB keeps sending UTF-8. §7.10.2.6 leaves that open; the empty list is what keeps it unreachable. A guard test fails if anyone grows the list, and names the probe to re-run first |
