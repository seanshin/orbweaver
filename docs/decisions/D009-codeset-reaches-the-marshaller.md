...
| 4 ◐ | ~~L2's seven publish sites~~ (**landed early**, `b9910f6` — a byte comparison between a generated skeleton and its hand-written servant went red the moment one published `TAG_CODE_SETS` and the other did not, so the cause landed whole rather than two sites at a time); **remaining**: a non-empty `char` conversion list | a peer advertising ISO-8859-1 **without** UTF-8. If none can be produced here, the list stays empty and the batch reports **BLOCKED** — advertising a conversion we cannot demonstrate is the defect this document exists to avoid |
