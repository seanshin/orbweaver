...
| **I4. Promotion respects identity** ◐ | B × C | A promoted static path carries the same `Caller` assertion behaviour as the dynamic path it replaced; "promotion dropped the identity context" is the regression gate's first test. **Gate landed**: `verify_promotion` checks `IdentityDropped` before comparing results, unit-verified with guard fakes; the live half (a real promotion's two audits through the gate against a peer) is not yet wired and is said so in the module. |
